...
let sliderInterval = Number(getComputedStyle(document.documentElement).getPropertyValue('--GS__slider-interval'));
// let baseSize = Number(getComputedStyle(document.documentElement).getPropertyValue('--GS__base-size').replace('px', ''));
let baseSize = eval(getComputedStyle(document.documentElement).getPropertyValue('--GS__base-size').replace(/[pxcalc\(\)]/g, ''));
let cardRatio = Number(getComputedStyle(document.documentElement).getPropertyValue('--GS__card-ratio'));
let animationTime = getComputedStyle(document.documentElement).getPropertyValue('--GS__animation-time');
...
function setPositions () { //resets positions of gallery object when needed
  ...
  // baseSize = Number(getComputedStyle(document.documentElement).getPropertyValue('--GS__base-size').replace('px', ''));
  baseSize = eval(getComputedStyle(document.documentElement).getPropertyValue('--GS__base-size').replace(/[pxcalc\(\)]/g, ''));
  ...
  slider.style.width = baseSize + (baseSize / cardRatio * 2) * (containers.length - 1) + "px";
}
...
function shiftSlider (dir) {
  if ((dir === "right" && bigCard < containers.length - 1) || (dir === "left" && bigCard > 0)) {
    ...
    containers[bigCard].style.flex = '0 0 calc(var(--GS__base-size) / var(--GS__card-ratio))';
